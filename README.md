...
| `flexible` | Generator or solar, whichever's around. Free if it lands on grid or solar time; only costs fuel as a last resort. |
...
to go; flexible and solar jobs grab whatever grid time is left over, because that's
free; whatever's left of the solar-covered cut time goes to solar-only jobs first (it's
their one shot), with flexible jobs taking what remains; anything flexible still
unplaced falls back to the generator, but only if you actually have one; and jobs that
need no power at all soak up cut time first since that space would otherwise sit idle.
...
If you try to put a solar-only job somewhere solar can't reach, it just tells you no —
it won't quietly bill that to the generator instead.
...
- **Sign up / sign in** — email and password, or "Continue with Google" if you've set
  up a Client ID (see below). No Client ID, no Google button — nothing breaks either way.
...
- **Plan view** — a timeline with cuts shaded and jobs colored by what actually powered
  them, a scrubber, a small 3D scene (generator, solar panel, mains pylon — the panel
  only shows up if you have solar) that lights up whatever's actually running at the
  scrubbed time, a table for manual start-time overrides and attaching a document to a
  job, and buttons to reset to the auto-plan or save your own arrangement.
...
seed data — it's your shop, so you fill it with your own jobs.
...
You only need a public Client ID here, no client secret — the backend checks the
...
1. Go to [console.cloud.google.com](https://console.cloud.google.com), make or pick a
   project, then **APIs & Services → OAuth consent screen** → External → fill in the
   basics and save. Testing mode is fine for personal use.
...
2. Set the real env vars on your host: `DATABASE_URL`, `JWT_SECRET`,
   `FRONTEND_ORIGIN` (has to match your deployed frontend exactly — CORS is locked to
   one origin, not `*`), and `GOOGLE_CLIENT_ID` if you're using it.
3. If you want job attachments to survive a redeploy, attach persistent storage for
   `backend/uploads/` — most Node hosts wipe the filesystem on every deploy.
...
`VITE_GOOGLE_CLIENT_ID` if relevant) to match your backend — these get baked into the
JS at build time, so changing them later means a rebuild, not just a restart.
...
- Rate limiting on `/auth/signup`, `/auth/login`, and `/auth/google` specifically
  (to blunt brute-forcing), plus a looser limit across the whole API.
- Every plan, job, and meter reading route checks that you actually own the resource
  before returning anything, and returns a plain 404 either way — it never lets you
  tell the difference between "not yours" and "doesn't exist."
- Uploads are capped at 20MB, filtered by type, and stored under a random filename, not
  whatever the client sent, so there's no path-traversal angle.
- Every input gets validated with Zod before it touches the database.

## What's not done

- No password reset for email/password accounts (Google accounts don't need it — Google
  handles that side).
...
- No email verification or real account recovery — fine for a personal tool, not fine
  for handling strangers' accounts at scale.
